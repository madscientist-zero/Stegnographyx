...
class Steganography(object):
    @classmethod
    def encode(cls, inputs, outputs, encode_text):
        """
        hide text to image
        ...
        hide_text(outputs, encode_text)
        assert read_text(outputs) == encode_text, read_text(outputs)
...
def main():
    if len(sys.argv) == 5 and sys.argv[1] == '-e':
        print("\033[31m"+"STEGANOGRAPHY")
        print("\033[32m"+"------------------")
        print("\033[41m"+"Now Encoding...")
        inputs = sys.argv[2]
        outputs = sys.argv[3]
        text = sys.argv[4]
        Steganography.encode(inputs, outputs, text)
        print("\033[41m"+"Finish:{}".format(outputs))
        return
    if len(sys.argv) == 3 and sys.argv[1] == '-d':
        inputs = sys.argv[2]
        print(Steganography.decode(inputs))
        return
    print_help_text()


def print_help_text():
    print("\033[41m"+"not steganography command")
    print("\033[31m"+"-----------------------------------------")
    print("# encode example: hide text to image")
    print("steganography -e input.jpg output.jpg 'My name is MadScientist'")
    print("")
    print("# decode example: read secret text from image")
    print("steganography -d output.jpg")
    print("")
